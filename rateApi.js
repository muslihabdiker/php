...
const ticksResponse = async (res) => {
  const data = JSON.parse(res.data);
  var Askprice = parseFloat(data.tick).toFixed(0);
  var Bidprice = parseFloat(data.tick).toFixed(0);
  var Askscreen = document.getElementById("ask");
  var Bidscreen = document.getElementById("bid");
  Askscreen.innerHTML = Askprice;
  Bidscreen.innerHTML = Bidprice;
  ...
  Askscreen.style.color =  "green";
  Bidscreen.style.color =  "red";
};
